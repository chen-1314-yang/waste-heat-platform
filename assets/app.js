...
/* ORC 效率表覆盖层 —— 由 tools/gen_eff_table_override.py 生成，勿手改。

替换老站遗留的 HFDATA.orcPct。改了统计口径，不是偷偷换数据：

  旧口径：蒸发器出口 ≤ T 的**全部**工况取中位数（累计）——曲线被拉平，
          150~350℃ 的效率几乎不变，不再响应温度。
  新口径：**该温度档**全部可行工况的 P10/P50/P90（与内核蒸汽曲线同口径，
          decision_core.py 里那次 P2 修正就是这么改的）。

单位：kW 净输出 / MW 回收热；列 = [蒸发器出口温度℃, P10, P50, P90, 该档工况数]
数据：自写 CoolProp 循环模型，5,925 工况（异丁烷；压力 1.8~3.6 MPa、
      背压 0.5~0.7 MPa、膨胀机等熵效率 0.70~0.90、泵效率 0.80；
      已按"出口温度 ≥ 露点 +2K"过滤）。非实测。
独立复算：DWSIM（第三方流程模拟器）净功率中位偏差 −1.28%，98.6% 的点 ≤2%；
      热效率不可直接比 —— DWSIM 模板把泵入口固定在 35℃，其吸热量不随背压变化。
      详见 04_仿真数据与图表\dwsim_full_verification.md。
生成日期：2026-09-20
*/
...
基准来自内嵌的仿真分位表（HFDATA.orcPct，26 行，温度 100~350℃），
单位是「每 MW 回收热的净功率 kW」。可学习的部分是叠加在上面的一个偏置修正。

注意：这张表由 src/legacy/eff_table_override.js 覆盖，口径是「同一蒸发器
出口温度档取中位数」，与内核蒸汽曲线那次 P2 修正一致；老站遗留的 orcPct
是「累计中位数」，曲线被拉平、不响应温度，已不再使用。

注意：基准 P50 在 150~350℃ 区间本身并不单调（有 20 处逆序），
这是数据的事实（压力上限固定、过热段变长），因此"单调性"门控只能定义为
"更新不得让它变得更差"，不能定义为"必须单调"。
*/
window.WHLAB_MODEL = (function () {
  ...
  function calibrationSet() {
    /* 键名叫 real14，但它**不是实测数据**：它是同一条自研模型链路跑出来的
       帕累托最优解（核对_第五轮_前端与清理.md：pymoo / steam / real14
       都是"帕累托解"，14 条）。因此 calibrationError 衡量的是
       "设计空间中位设计" 与 "前沿最优设计" 之间的差距，不是模型误差。
       键名沿用的是提取脚本里的历史命名，不要据此写成"实测"。 */
    return (window.HFDATA && window.HFDATA.real14) || [];
  }

  /* 在前沿留出解上的平均绝对相对偏差（口径见上，不是对实测的误差） */
...
window.VIEW_AFTER.learning = function () {
  ...
  function renderState() {
    ...
    el("lab-storage").innerHTML =
      ...
      '<span>基准：内嵌仿真分位表 P50（26 行，100~350 ℃，按温度档中位数）</span>' +
      '<span>留出复核集：' + M.calibrationSetSize() + ' 个帕累托解</span>';
    var baseErr = M.calibrationError(null);
    if (baseErr !== null) {
      el("lab-storage").innerHTML +=
        '<span>基准（中位设计）对帕累托前沿（最优设计）的平均偏差：<strong>' +
        (baseErr * 100).toFixed(2) + '%</strong>（两者口径不同：一个是设计空间' +
        '中位、一个是前沿最优，所以基准系统性更低。这不是对实测的误差）</span>';
    }
  }
